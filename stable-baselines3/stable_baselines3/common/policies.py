...
class ActorCriticPolicy(BasePolicy):
    ...
    def forward(self, obs: th.Tensor, mask, deterministic: bool = False) -> Tuple[th.Tensor, th.Tensor, th.Tensor]:
        """
    ...
    def _get_action_dist_from_latent(self, latent_pi: th.Tensor, mask=None) -> Distribution:
        ...
        #         th.finfo(mean_actions.dtype).min, dtype=mean_actions.dtype
        #     )
        # mask_value = mean_actions.min()
        # mean_actions = th.where(mask.reshape(-1), mean_actions, mean_actions.min())
        # 概率化mean_actions
        # mean_actions = th.softmax(mean_actions)
        if isinstance(self.action_dist, DiagGaussianDistribution):
            ...
            return self.action_dist.proba_distribution(mean_actions, None)
    ...
    def _predict(self, observation: PyTorchObs, deterministic: bool = False, mask=None) -> th.Tensor:
        """
        Get the action according to the policy for a given observation.
